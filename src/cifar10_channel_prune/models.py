...
class Model(object):
  def __init__(self,
    ...
    with tf.device("/cpu:0"):
      ...
      if images["valid"] is not None:
        ...
        assert self.num_valid_examples % eval_batch_size == 0
        self.num_valid_batches = self.num_valid_examples // self.eval_batch_size
      ...
      #  // self.eval_batch_size)
      assert self.num_test_examples % eval_batch_size == 0
      self.num_test_batches = self.num_test_examples//self.eval_batch_size
  ...
  def eval_once(self, sess, eval_set, feed_dict=None, verbose=False):
    ...
    #self.eval_once_acc = float(total_acc)/total_exp
    return 100.0*float(total_acc)/total_exp
