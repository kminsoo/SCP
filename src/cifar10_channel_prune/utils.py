...
def get_train_ops(
  ...
  if l2_reg > 0:
    l2_losses = []
    for var in tf_variables:
        if "offset" not in var.name: # should exclud bias for weight decay
  ...
  if lr_cosine:
    ...
    if not child_lr_otf:
      ...
      epoch_1,epoch_2 = 80,120

      curr_epoch = train_step // num_train_batches
      default_fn = lambda:lr_init * 0.1
      learning_rate = tf.case({tf.less(curr_epoch, epoch_1):lambda: lr_init, tf.greater(curr_epoch,epoch_2): lambda: lr_init * 0.01}, default=default_fn)
