...
class densenet(Model):
  ...
  def add_layer(self,inputs, is_training):
    ...
    in_channel = np.sum(inference_mask)
    argmax_mask = np.where(inference_mask)[0]

    outputs = batch_norm_init(inputs, is_training, reader=self.reader,
                              name='bn', data_format=self.data_format)
  ...
  def _model(self, images, is_training, reuse=False):
    ...
    with tf.variable_scope(self.name, reuse=reuse) as scope:
      ...
      with tf.variable_scope('fc') as scope:
        ...
        l = batch_norm_init(l, is_training, reader=self.reader,name='bn',
  ...
  def _build_train(self):
    ...
    original_flops=566774668
    original_params=1059298
    original_channels=9360
