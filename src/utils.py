...
def get_train_ops(
  ...
  if lr_cosine:
    ...
    if not child_lr_otf:
      ...
      """
      if imagenet_lr:
        epoch_1, epoch_2 = 30, 60
      else:
        epoch_1, epoch_2 = 150, 224
      """
      epoch_1,epoch_2 = 150,224

      curr_epoch = train_step // num_train_batches
      default_fn = lambda:0.01
      learning_rate = tf.case({tf.less(curr_epoch, epoch_1):lambda: 0.1, tf.greater(curr_epoch,epoch_2): lambda: 0.001}, default=default_fn)
  ...
  if not ensemble:
    ...
    train_op = opt.apply_gradients(
    zip(grads, tf_variables), global_step=None)
